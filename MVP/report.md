...
Concrete files in the current MVP include:

- `portugal_apartment_rent_benchmarks.json` with 6 sample records
- `portugal_real_estate_2024_sample.json` with 6 sample records
- `germany_rentals_sample.json` with 6 sample records
- `germany_house_purchase_benchmarks.json` with 6 sample records
- `japan_apartment_rent_benchmarks.json` with 6 sample records
- `japan_property_transactions_sample.json` with 6 sample records
...
NestGPT is an agentic workflow built around a hosted frontier language model rather than a fine-tuned checkpoint. In the current implementation, the default hosted model is `openai/gpt-5.4`, with the option to override it through the `OPENROUTER_MODEL` environment variable.
...
The current qualitative comparison target between baseline mode and research mode is summarized below:

| Criterion | Baseline AI Mode | NestGPT Research Mode |
| --- | --- | --- |
| Source citations | Usually absent | Present when retrieval succeeds |
| City-specific affordability context | More generic | Stronger because local web and dataset evidence are injected |
| Rental-process detail | Often broad | More specific to the selected city and use case |
| Uncertainty handling | Depends on model-only response | Stronger because the workflow explicitly surfaces weak or conflicting evidence |

In practical terms, the retrieval-backed mode is intended to outperform baseline mode on:
...
This MVP does not yet include an automated evaluation harness or numeric accuracy benchmark. The current evaluation is still qualitative and based on benchmark-case comparison rather than scored metrics. A stronger next phase would save benchmark outputs and score them across fixed rubrics such as:
